  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setErr(e?.response?.data?.detail || "Página não disponível");
      }
    })();
  }, []);

  const resolveUrl = (path) => path?.startsWith("/api/uploads/file/")
    ? `${BACKEND}/api/public/uploads/${path.replace("/api/uploads/file/", "")}`
    : `${BACKEND}${path}`;
  ...
  return (
    <div className="min-h-screen bg-white">
      <LandingPreview
        ...
        renderSpecification={data.render_specification}
      />
    </div>
  );
}