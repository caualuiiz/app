...
import {
  Bot, Camera, ExternalLink, Loader2, Send, Sparkles, Upload,
} from "lucide-react";
...
export default function LandingBuilderPage() {
  ...
  const send = async (text) => {
    ...
    try {
      ...
      if (r.ask_photos && window.innerWidth > 768) toast.info("💡 Você pode enviar fotos abaixo do chat");
    } catch (e) { toast.error(formatApiError(e)); }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-slate-600">Crie sua página conversando com nossa IA</span>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-4 h-[calc(100vh-220px)]">
          <div className={`bg-white border border-slate-200 rounded-xl flex flex-col ${tab === "preview" ? "hidden md:flex" : "flex"}`}>
            <div className="border-b border-slate-100 p-3 flex items-center gap-2"><Bot className="h-4 w-4 text-indigo-600"/><span className="font-semibold text-sm">Assistente IA</span></div>
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3" data-testid="chat-messages">
              ...
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm whitespace-pre-wrap ${m.role === "user" ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-800"}`}>{m.content}</div>
                </div>))}
              {sending && <div className="flex justify-start"><div className="bg-slate-100 rounded-2xl px-4 py-2"><Loader2 className="h-4 w-4 animate-spin text-slate-400"/></div></div>}
            </div>
            <div className="border-t border-slate-100 p-3 space-y-2">
              ...
              {data?.state?.gallery?.length > 0 && <div className="flex gap-1 overflow-x-auto"><Badge variant="outline">{data.state.gallery.length} fotos</Badge></div>}
            </div>
          </div>

          <div className={`bg-slate-50 border border-slate-200 rounded-xl overflow-hidden ${tab === "chat" ? "hidden md:block" : "block"}`} data-testid="landing-preview">
            {data && <LandingPreview state={data.state} company={company}/>}
          </div>
        </div>
      </div>
    </AppShell>
