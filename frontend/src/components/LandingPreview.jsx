...
export default function LandingPreview({ state, company, publicMode = false, services = [], resolveUrl }) {
  ...
  const ImgTag = publicMode ? ({ path, className }) => (
    <img src={resolveUrl ? resolveUrl(path) : path} className={className} alt=""/>
  ) : AuthedImg;

  return (
    <div className="h-full overflow-y-auto bg-white text-slate-900" data-testid="preview-root">
      {sec.hero !== false && (
        <section className="relative min-h-[380px] md:min-h-[480px] flex items-center px-6 md:px-12 py-16 text-white" style={{ background: `linear-gradient(135deg, ${primary} 0%, ${primary}dd 100%)` }}>
          {company?.establishment_photo_url && (
            <div className="absolute inset-0 opacity-30">
              <ImgTag path={company.establishment_photo_url} className="w-full h-full object-cover"/>
            </div>
          )}
          <div className="relative max-w-3xl">
            ...
            <button className="bg-white text-slate-900 px-6 py-3 rounded-lg font-semibold hover:bg-slate-100 transition">{s.hero?.cta || "Agendar horário"}</button>
          </div>
        </section>
      )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {s.differentiators.map((d, i) => (
                <div key={i} className="bg-white rounded-xl p-5 border border-slate-200">
                  <div className="h-8 w-8 rounded-lg mb-3" style={{ background: `${primary}22`, color: primary, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700 }}>{i+1}</div>
        ...
        <section className="px-6 md:px-12 py-12">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {services.map(sv => (
                <div key={sv.id} className="border border-slate-200 rounded-xl p-5 hover:shadow-md transition">
      ...
      <footer className="px-6 md:px-12 py-6 bg-slate-900 text-slate-400 text-center text-sm">
